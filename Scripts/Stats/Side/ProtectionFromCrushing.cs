...
using Stats.SideStatsProvider.ProtectionFromCrushing;
using UnityEngine;

namespace Stats.Side
{
    public class ProtectionFromCrushing: IPhysicalResist
    {
        ...
        public ProtectionFromCrushing(IBasicStats basicStats, Level level, SideStatsValueFactory valueFactory,
            IPolicyThatStatsIsFilled policyThatStatsIsFilled, IPolicyThatStatsIsOver policyThatStatsIsOver)
        {
            _basicStats = basicStats;
            _level = level;
            _sideStatProvider = new ProtectionFromCrushingProvider(basicStats, level, valueFactory);
