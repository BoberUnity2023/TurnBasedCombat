...
using Stats.SideStatsProvider.ProtectionFromIce;

namespace Stats.Side
{
    public class ProtectionFromIce : IMagicResist
    {
        ...
        public ProtectionFromIce(IBasicStats basicStats, Level level, SideStatsValueFactory valueFactory,
            IPolicyThatStatsIsFilled policyThatStatsIsFilled, IPolicyThatStatsIsOver policyThatStatsIsOver)
        {
            _basicStats = basicStats;
            _level = level;
            _sideStatProvider = new ProtectionFromIceProvider(basicStats, level, valueFactory);
